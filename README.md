...
| **`grav-admin-ui-polish`** | You're reviewing or polishing a plugin's Admin Next UI — a plugin page, an add-on section, a dashboard widget, or its blueprint settings form — and want it to read as native and finished: the review method (both themes, tablet width, read-only), the ten design rules (status registry instead of raw enums, the zero-value rule, one primary per screen, ⋯ row menus, every editor a route with a dirty guard…), host fit against Admin Next's tokens, the primitives a plugin admin needs, the `ctx.ui` / `ctx.sheet` contract for add-on sections, blueprint label and help rules, and a twelve-item rendering-defect checklist. Pairs with `grav-api-admin-next-integration` (that one is how to register the UI; this one is what it should look like). |
...
| **`grav-2-migration`** | You're migrating an existing Grav 1.7/1.8 site to Grav 2.0 **by hand**, in place — core swapped via git or a deploy pipeline rather than staged and promoted by the `migrate-grav` plugin. Encodes the same transforms the plugin's wizard performs: classic `admin` → `admin2` + `api`, plugin/theme 2.0 compatibility verdicts, classic `admin.*` → `api.*` account and group permissions (including the `api.access` gate nothing in 1.x maps to), the `security.twig_content` gates and `twig_sandbox` allowlists, `system.images.url_actions`, and the GFM `tagfilter`. Ships two standalone scripts: a read-only scanner and a permissions rewriter. |
...
`grav-2-migration` is the odd one out: it's about migrating a *site*, not building a plugin. Reach for it when the thing in front of you is somebody's existing 1.7 install rather than a plugin repo — and note that for a plain hosted site with no version control, the [`migrate-grav` plugin](https://github.com/getgrav/grav-plugin-migrate-grav) is still the safer path, since it stages the new install alongside the old one instead of changing it in place.

Most plugin developers will want `grav-api-admin-next-integration`. Reach for the narrower `grav-api-integration` only when you're sure no admin UI work is needed. Reach for `grav-admin-ui-polish` once the UI exists and needs to look like it belongs in the admin. Pull in `grav-translations` whenever blueprint or admin labels are involved. Reach for `grav-plugin-review` when you're on the receiving end: vetting someone else's plugin rather than building your own.
